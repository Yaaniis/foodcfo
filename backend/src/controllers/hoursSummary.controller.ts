...
import { computeHoursSummary, type WorkedShiftInput } from '../lib/hoursSummary';
...
export async function exportHoursSummaryCsv(req: Request, res: Response) {
  ...
  const shiftAssignments = await prisma.shiftAssignment.findMany({
    where: {
      schedule: { restaurantId: req.user!.restaurantId, status: 'VALIDATED' },
  ...
  const summaries = computeHoursSummary(shifts);
